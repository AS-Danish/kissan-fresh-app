...
class SlotSelectionController extends GetxController {
  ...
  Future<void> fetchSlots() async {
    ...
    try {
      ...
      final snap = await _firestore.collection('slots')
          .where('endTime', isGreaterThan: Timestamp.fromDate(now))
          .orderBy('endTime', descending: false)
          .limit(50)
          .get();

      availableSlots.value = snap.docs.map((doc) => SlotModel.fromJson(doc.data(), id: doc.id)).toList();
      ...
      if (selectedSlotId.value.isNotEmpty) {
         final slotExists = availableSlots.any((s) => s.id == selectedSlotId.value);
  ...
  List<SlotModel> get slotsForSelectedDate {
     return availableSlots.where((slot) => _formatDateGroup(slot.startTime) == selectedDateLabel.value).toList();
  }
}
